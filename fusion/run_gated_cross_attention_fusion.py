...
class GatedCrossAttentionFusion(nn.Module):
    def __init__(
        ...
        # Temporary classifier used only during training.
...
def main():
    ...
    parser.add_argument("--use_scheduler", action="store_true",
                        help="Enable cosine annealing LR scheduler. Off by default.")
    ...
    train_dataset = EmbeddingDataset(
        train_df, args.wsi_dir, args.clinical_dir, args.wsi_dim, args.clinical_dim,
    )
    val_dataset = EmbeddingDataset(
        val_df, args.wsi_dir, args.clinical_dir, args.wsi_dim, args.clinical_dim,
    )
    test_dataset = EmbeddingDataset(
        test_df, args.wsi_dir, args.clinical_dir, args.wsi_dim, args.clinical_dim,
    )
    ...
    # Cosine annealing (only if --use_scheduler is passed).
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        val_loss, val_auc, val_acc, val_f1 = evaluate(model, val_loader, criterion, device)
        ...
        # LR schedule (only active when --use_scheduler is passed).
        if args.use_scheduler:
    ...
    export_fused_embeddings(model, train_dataset, output_dir / "fused_train", device)
    export_fused_embeddings(model, val_dataset,   output_dir / "fused_val",   device)
    export_fused_embeddings(model, test_dataset,  output_dir / "fused_test",  device)

    summary_path = output_dir / "fusion_summary.txt"
    with open(summary_path, "w", encoding="utf-8") as f:
        f.write("Gated Cross-Attention Fusion Summary\n")
        f.write("=====================================\n\n")
        ...
        f.write(f"Scheduler: {'cosine+warmup' if args.use_scheduler else 'none (flat LR)'}\n")
